...
class Tides:
    def __init__(self, station_id,date,interval,samples,datum="MLLW"):
        self.data = requests.get(
            f"https://api.tidesandcurrents.noaa.gov/api/prod/datagetter?begin_date="+date+"&range="+samples+"&station="+station_id+"&product=predictions&datum="+datum+"&time_zone=gmt&interval="+interval+"&units=english&format=json", timeout=5).json()
        requests.session().close()
        pass

    def update(self, station_id,date,interval,samples):
        self.data = requests.get(
            f"https://api.tidesandcurrents.noaa.gov/api/prod/datagetter?begin_date="+date+"&range="+samples+"&station="+station_id+"&product=predictions&datum="+datum+"&time_zone=gmt&interval="+interval+"&units=english&format=json", timeout=5).json()
        requests.session().close()
        return self.data
...
class Display:
    ...
    def draw_text_centered(self, ypos, text, font, color="b"):
        w, h = self.draw_black.textsize(text,font)
        #print(w,h) # debug
        if color == "b":
            self.draw_black.text(((800-w)/2, ypos), text, fill=0, font=font)
        elif color == "r":
            self.draw_red.text(((800-w)/2, ypos), text, fill=0, font=font)
...
#        print(r,r2) # debug
    ...
    def show_pic(self, url, pos_x, pos_y, color="b", nourl=1):
        if nourl != 0:
